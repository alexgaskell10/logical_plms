## An adaption of the BERT scripts from huggingface
...
##########################
# SICK SPECIFIC READERS  #
##########################

def _read_orig_sick(file_loc,run_type):
    """reads the original sick file
    
    :param file_loc: the location of the original file 
    :param run_type: the type of data to extract
    """
    lines = [] 
    with open(file_loc) as sick:
        for k,line in enumerate(sick):
            if line == 0: continue
            idx,sen_a,sen_b,entailment,_,_,_,_,_,_,_,dset = line.strip().split("\t")
            entailment = entailment.lower().strip()
            if dset != run_type: continue
            lines.append([idx,sen_a,sen_b,entailment])
    logger.info('Parsed sick %s, size=%d' % (run_type,len(lines)))
    return lines

def _read_modified_sick(sick_path,with_arrows,run_type):
    ...
    ## find the original ones, to start experimenting with original non-lemmatized versions 
    with open(full_lemmatized) as lemmatized:
        with open(full_raw) as raw:
            l = lemmatized.readlines()
            r = raw.readlines()
            assert len(l) == len(r)
    ...
    ## map onto
    file_path = {
        "TRAIN" : os.path.join(sick_path,"sick.train.all_labels.polarized.tsv"),
        "TRIAL" : os.path.join(sick_path,"sick.trial.all_labels.polarized.tsv"),
        #"TEST"  : os.path.join(sick_path,"sick.test.all_labels.polarized.tsv"), ## comment it out just 
    }

    target_file = file_path.get(run_type,None)
    if target_file is None:
        raise ValueError('Cannot find version of SICK data: %s' % run_type)

    lines = []
    ## now parse the target file
    with open(target_file) as target:
        for k,line in enumerate(target):
            line = line.strip()
            idx,sen_a,sen_b,entailment,rset = line.split("\t")
            entailment = entailment.lower().strip()

            assert rset == run_type



            ## still need to convert back to unlemmatized
            sen_a = raw_map[sen_a]
            sen_b = raw_map[sen_b]
            
            # if with_arrows:
            #     ## I think the bert tokenizer will do this anyway
            #     ## adds space
            #     sen_a = sen_a.replace("↑"," ^")
            #     sen_a = sen_a.replace("↓"," ↓")
            #     sen_a = sen_a.replace("="," =")
...
class SICKProcessor(RteProcessor):
    ...
    def get_train_examples(self, data_dir,orig=True,with_arrows=False,lemmatized=False):
        """See base class."""
        ## the case of original case        
        if orig:
            dataset = os.path.join(data_dir,"orig/SICK.txt")
            return self._create_examples(_read_orig_sick(dataset,"TRAIN"),"train")

        ## the case of arrow data
        else:
            return self._create_examples(_read_modified_sick(data_dir,with_arrows,"TRAIN"),"train")

    def get_dev_examples(self, data_dir,orig=True,with_arrows=False):
        """See base class."""
        if orig:
            dataset = os.path.join(data_dir,"orig/SICK.txt")
            return self._create_examples(_read_orig_sick(dataset,"TRIAL"),"dev")
        else:
            return self._create_examples(_read_modified_sick(data_dir,with_arrows,"TRIAL"),"dev")
    ...
    def _read_tsv(cls, input_file,quotechar=None):
        """Reads a tab separated value file."""
        raise NotImplementedError('not implemented for SICK')
...
def parse_args():
    ...
    parser.add_argument("--inoculate",
                        default='',
                        help="Inoculate the model with challenge dataset (should container pointer to existing fine-tuned model)")
    ...
    ###################################
    # ## SICK SPECIFC SETTINGS        #
    ###################################

    parser.add_argument("--with_arrows",
                        action='store_true',
                        help="Run an experiment with arrows annotations as tokens")

    parser.add_argument("--sick_dev",
                        action='store_false',
                        default=True,
                        help="Run sick experiment with arrows as tokens")

    parser.add_argument("--sick_orig",
                        action='store_false',
                        help="Run sick experiment with arrows as tokens")

    parser.add_argument("--sick_mod",
                        action='store_true',
                        help="Run sick experiment with arrows as tokens")

    parser.add_argument("--sick_data",
                        default='',
                        help="The location of the sick data")
...
def main():
    ...
    if args.inoculate:
        if not args.bert_config:
            raise ValueError('To inoculate a model, you must provide existing model and configuration via --bert_config!')
        config = BertConfig(args.bert_config)
        model = BertForSequenceClassification(config, num_labels=num_labels)
        model.load_state_dict(torch.load(args.inoculate))
    ...
    ##########################
        ...
        if not args.bert_config:
            raise ValueError('Must provide a pointer to BERT configuration via --bert_config')
        config = BertConfig(args.bert_config)
        model = BertForSequenceClassification(config, num_labels=num_labels)
        model.load_state_dict(torch.load(args.run_existing))
